	...
	void insertTestData() {
		...
		new File("maps_index.txt").eachLine { line, nr ->
			...
			if (nr <= TESTDATA_IMPORT_COUNT && !Map.findByTilesetUrl(mapUri)) {
			  ...
			  def date = new Date();
				new Map(tilesetUrl: mapUri, user: adminUser, name: name, views: 0, description: "Description", uploadDate: date, editDate: date, mapDate: date, isPublic: true).save(flush: true)
			}
		}
	}
